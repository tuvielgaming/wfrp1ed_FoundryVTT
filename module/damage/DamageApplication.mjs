...
export class DamageApplication {
	...
	/**
	 * Persist the first resolution of a critical generated by one already-applied
	 * damage transaction. Ordinary resolution is idempotent: once recorded it is
	 * returned unchanged. Deliberate GM adjudication uses replaceCriticalResolution
	 * below instead of pretending that a second random roll occurred.
	 */
	static async recordCriticalResolution({
		...
		return this.#writeCriticalResolution(
			actor,
			id,
			existing,
			criticalResolution,
	...
	/**
	 * Replace only the table-resolution snapshot after an explicit GM edit of an
	 * already resolved critical d100. Remaining Wounds and the damage transaction
	 * itself are not replayed. Callers must separately synchronize any materialized
	 * Critical Wound/Fatal-result presentation which belongs to the old result.
	 */
	static async replaceCriticalResolution({
		actor,
		packetId,
		criticalResolution,
		user = game.user,
		...
		if (!(actor instanceof foundry.documents.Actor)) {
			throw new Error("Critical adjudication requires a target Actor.");
		}
		if (!user?.isGM) {
			throw new Error(
				"Only a GM may replace an already-resolved critical result.",
			);
		}
		...
		if (!existing || existing.state !== "applied") {
			throw new Error(
				"Critical adjudication requires an applied damage transaction.",
			);
		}
		if (!existing.criticalResolution) {
			throw new Error(
				"There is no resolved critical result to adjudicate.",
			);
		}
	...
	static async #writeCriticalResolution(
		actor,
		id,
		existing,
		criticalResolution,
